...
public:
	...
	ZakharovBenchmark(std::vector<Bound<T>> &bounds) : Benchmark<T>("Zakharov function",0)       {
		copy(bounds.begin(), bounds.end(), back_inserter(this->mBounds));
        	for (int i = 0; i < bounds.size(); i++) {
			this->mGlobMinX.push_back(0);
                        //this->mBounds.push_back(bounds[i]);
		}
	}
};
template <class T>
